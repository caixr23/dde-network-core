...
void NetManagerPrivate::onDataChanged(int dataType, const QString &id, const QVariant &value)
{
    ...
    case NetManagerThreadPrivate::ConnectionStatusChanged: {
        ...
        qWarning() << __FUNCTION__ << __LINE__ << item->name() << item->status();
    } break;
    ...
    case NetManagerThreadPrivate::DetailsChanged: {
        ...
        qWarning() << __FUNCTION__ << __LINE__ << item << id << findItem(id);
        if (item)
            item->updatedetails(value.value<QList<QStringList>>());
        {
            ...
            while (!items.isEmpty()) {
                auto i = items.takeFirst();
                qWarning() << __LINE__ << __FUNCTION__ << i->id() << i->name() << i->itemType() << i << i->getParent();
                items.append(i->getChildren());
            }
        }
    } break;
...
{
    qWarning() << __FUNCTION__ << __LINE__ << item << parentItem;
    NetItemPrivate *oldItem = findItem(item->id());
    if (!oldItem) {
        m_dataMap.insert(item->id(), item);
        qWarning() << __LINE__ << "addItem" << item->id() << item->name();
